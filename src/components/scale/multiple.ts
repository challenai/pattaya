...
export function fragments(
    { segements, height, padding, vertical, arrowSize }: MultipleScaleProps,
    styles: ScaleStyles,
): MultipleScaleFragments {
    ...
    for (const item of segements) {
        ...
        lineFrag.shapes!.push({
            path: wireframeLine(
                offset + item.width - padding - lineWidth,
...
export function applyStyles(
    fragments: MultipleScaleFragments,
    styles: ScaleStyles,
) {
    const optsArrow = toOptsArrow(styles);
    const optsBorder = toOptsBorder(styles);
