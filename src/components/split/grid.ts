...
function buildVerticalSeperators(
    width: number,
    height: number,
    units: GridUnit[],
): string {
...
function buildHorizontalSeperators(
    width: number,
    height: number,
    units: GridUnit[],
): string {
...
export function shapes(
    { width, height, units, vertical, radius }: GridProps,
    styles: SplitStyles,
): Shapes {
    return [
        {
            path: radius
                ? rectangle.roundAligned(0, 0, width, height, radius)
                : rectangle.basicAligned(0, 0, width, height),
            opts: toBoxOpts(styles),
        },
        {
            path: vertical
                ? buildVerticalSeperators(width, height, units)
                : buildHorizontalSeperators(width, height, units),
...
export function update(
    ...
    shapes![0].path = radius
        ? rectangle.roundAligned(0, 0, width, height, radius)
        : rectangle.basicAligned(0, 0, width, height);
    shapes![1].path = vertical
        ? buildVerticalSeperators(width, height, units)
        : buildHorizontalSeperators(width, height, units);
}
